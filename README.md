...
**Делает Claude:** обновляет версию и changelog, создаёт ветку релиза, открывает PR в `master`, мержит его, создаёт git-тег.

**Вручную (пользователь):** пушит созданный тег в `origin` — только это действие запускает деплой на WordPress.org. Push тегов заблокирован для Claude в этом окружении.
