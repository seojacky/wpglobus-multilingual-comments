...
1. Создаёт ветку с именем версии (например, `1.5.5`) **от текущей рабочей ветки Claude Code** (например, `claude/...`), в которой уже подготовлены изменения для релиза, а не от `master` напрямую — `master` может отставать, если наработки ещё не смержены.
2. Если правки под релиз (номер версии в шапке `wpglobus-multilingual-comments.php`, `Stable tag` и запись в `readme.txt` `== Changelog ==`) ещё не сделаны — Claude делает их перед созданием ветки версии.
3. Пушит ветку версии в репозиторий.
